...
async function viewDatabase() {
  try {
    ...
    generationResult.rows.forEach((row) => {
      console.log(
        `  ${row.dmonth.toISOString().split("T")[0]} | ${row.energy_type}: ${row.generation_gwh} GWh`,
      );
    });
    ...
    const initiativesResult = await pool.query(
      "SELECT * FROM state_initiatives ORDER BY state_id;",
    );
    initiativesResult.rows.forEach((row) => {
      console.log(
        `  ${row.state_id}: ${row.target_value_pct}% reduction by ${row.target_year} (baseline: ${row.baseline_year})`,
      );
    });
