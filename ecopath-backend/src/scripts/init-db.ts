          ...
          for (let j = 1; j < headers.length; j++) {
            ...
            if (!isNaN(generation) && generation > 0) {
              try {
                ...
                console.log(
                  `⚠️  Error inserting generation data for ${state}, ${parsedDate}, ${mappedEnergyType}: ${error}`,
                );
              }
            }
          }
  ...
  const initiatives = [
    {
      ...
      baseline_year: 2005,
      target_value_pct: 50,
      notes: "Net zero emissions target",
    },
    {
      state_id: "NSW",
      target_year: 2030,
  ...
  for (const initiative of initiatives) {
    ...
    `,
      [
        ...
        initiative.baseline_year,
        initiative.target_value_pct,
        initiative.notes,
      ],
    );
  }
