...
const mockNewsItems = [
  ...
  },
  {
    ...
    summary:
      "Nearly five years ago, the Royal Commission into National Natural Disaster Arrangements—sparked by the Black Summer bushfires—identified a major gap: Australia did not have a single, comprehensive source of climate risk information...",
  ...
  },
  {
    ...
    summary:
      'The Mid-North Coast of NSW is currently experiencing record-breaking flooding, after experiencing back-to-back extreme weather events in the last few years. It is critical to understand that these kinds of disasters are no longer simply "natural"...',
...
describe("Climate News API", () => {
  describe("GET /api/news/climate", () => {
    it("should return all climate news items", async () => {
      const response = await request(app).get("/api/news/climate").expect(200);
  ...
  describe("GET /api/news/climate/category/:category", () => {
    it("should return news items filtered by Critical category", async () => {
      const response = await request(app).get("/api/news/climate/category/Critical").expect(200);
    ...
    it("should return news items filtered by High Risk category", async () => {
      const response = await request(app).get("/api/news/climate/category/High Risk").expect(200);
    ...
    it("should return empty array for non-existent category", async () => {
      const response = await request(app).get("/api/news/climate/category/NonExistent").expect(400);
    ...
    it("should return 400 for invalid category", async () => {
      const response = await request(app).get("/api/news/climate/category/invalid").expect(400);
  ...
  describe("GET /api/news/climate/:id", () => {
    it("should return specific news item by ID", async () => {
      const response = await request(app).get("/api/news/climate/climate-1").expect(200);
    ...
    it("should return specific news item with flood content", async () => {
      const response = await request(app).get("/api/news/climate/climate-3").expect(200);
    ...
    it("should return 404 for non-existent news item", async () => {
      const response = await request(app).get("/api/news/climate/non-existent").expect(404);
