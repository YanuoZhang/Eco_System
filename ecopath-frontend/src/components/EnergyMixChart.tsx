...
export default function EnergyMixChart({ data, title = "Energy Generation Mix" }: EnergyMixChartProps) {
        ...
        >
          ...
          <div className="space-y-1">
            <p className="text-gray-600">
              <span className="text-2xl font-bold text-purple-600">{hoveredEnergy.percentage}%</span> of total generation
            </p>
            <p className="text-gray-500 text-sm">
              Capacity: <span className="font-medium text-gray-700">{hoveredEnergy.generation}</span>
            </p>
            <p className={`text-sm font-medium ${
              hoveredEnergy.trend > 0 ? 'text-green-600' : 'text-red-600'
            }`}>
              Trend: {hoveredEnergy.trend > 0 ? '+' : ''}{hoveredEnergy.trend}%
            </p>
          </div>
        </div>
      )}
