...
beforeAll(() => {
  class IO implements IntersectionObserver {
    ...
    observe() { this.cb([{ isIntersecting: true } as any], this); }
    disconnect() {}
    unobserve() {}
...
describe('Home page composition', () => {
  it('renders hero, news, timeline, CTA and footer; supports timeline switch and news flip', async () => {
    ...
    // News heading
    expect(screen.getByRole('heading', { name: /Latest Australian Climate Impact Updates/i })).toBeInTheDocument();
    ...
    expect(await screen.findByRole('heading', { name: /First Climate Signals/i })).toBeInTheDocument();
