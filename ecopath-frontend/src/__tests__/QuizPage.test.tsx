...
vi.mock('@/components/quiz/QuizHero', () => ({ default: ({ states, selectedState }: any) => (
  <div data-testid="quiz-hero">Hero {selectedState}</div>
) }));
vi.mock('@/components/quiz/QuizElectricity', () => ({ default: ({ onChange }: any) => (
  <button onClick={() => onChange({ electricityEmissionsKgYear: 1000 })}>elec</button>
) }));
vi.mock('@/components/quiz/QuizHotWater', () => ({ default: ({ onChange }: any) => (
...
vi.mock('@/components/quiz/QuizFloatingPreview', () => ({ default: ({ valueKgYear, onOpen }: any) => (
  <div>
    <span>preview:{valueKgYear}</span>
    <button onClick={onOpen}>open</button>
  </div>
) }));
vi.mock('@/components/quiz/QuizResultsModal', () => ({ default: ({ open }: any) => (
  open ? <div role="dialog">results</div> : null
) }));
