...
test.describe("Data Sources Workflow E2E Tests", () => {
  ...
  test("TC-1.7.2: Click button → Modal/panel opens with list", async ({ page }) => {
    ...
    await expect(
      page.getByText("All data sources are official government and research institutions"),
    ).toBeVisible();
  ...
  test("TC-1.7.5: All dataset links are clickable and have proper styling", async ({ page }) => {
    ...
    await expect(page.getByText("ABS Census Data")).toBeVisible();
    await expect(
      page.getByText("Australian Bureau of Statistics population and demographic data"),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Official climate change and emissions reduction data"),
    ).toBeVisible();
    await expect(page.getByText("Climate", { exact: true })).toBeVisible();

    await expect(page.getByText("AEMO Emissions Data")).toBeVisible();
    await expect(
      page.getByText("Australian Energy Market Operator energy and emissions data"),
    ).toBeVisible();
