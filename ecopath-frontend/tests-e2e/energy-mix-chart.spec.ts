...
test.describe('Energy Mix Chart E2E Tests', () => {
  ...
  test('TC-1.2.2: Hovering over chart segments shows tooltip with correct info', async ({ page }) => {
    // Navigate to homepage
    await page.goto('/');
    ...
    const coalSegment = page.locator('[data-testid="energy-mix-chart"] .bg-white.rounded-lg').first();
    ...
    await expect(page.locator('.absolute.z-50.bg-white')).toContainText('45.2% of total generation');
    await expect(page.locator('.absolute.z-50.bg-white')).toContainText('Capacity: 8,450 MW');
    
    // Hover over another energy source (Wind)
    const windSegment = page.locator('[data-testid="energy-mix-chart"] .bg-white.rounded-lg').nth(2);
    await windSegment.hover();
    
    // Verify tooltip updates
    await expect(page.locator('.absolute.z-50.bg-white')).toContainText('Wind');
    await expect(page.locator('.absolute.z-50.bg-white')).toContainText('22.8% of total generation');
  });
  ...
  test('Chart accessibility features work correctly', async ({ page }) => {
    ...
    const firstSegment = page.locator('[data-testid="energy-mix-chart"] .bg-white.rounded-lg').first();
